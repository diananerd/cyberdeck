...
## [Unreleased] — Clean-slate cutover to the 5-pillar spec

The DECK-3.0 framing was consolidated into the canonical Deck specification. Historical entries below this one describe work against prior spec iterations (`01-deck-lang.md` through `16-deck-levels.md`, the app annexes, the DL1/DL2 development plans) — those documents have been removed from the tree; the five pillar specs in `deck-lang/` (`LANG`, `SERVICES`, `CAPABILITIES`, `BUILTINS`, `BRIDGE`) are now the authoritative contract. When code and spec diverge, the spec wins.

Entries below remain as a historical record of the runtime's evolution. They reference files and concepts that no longer exist in the tree; consult git history for those artefacts.
