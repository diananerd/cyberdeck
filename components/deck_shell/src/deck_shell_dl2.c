...
static void launcher_card_settings_cb(lv_event_t *e)
{
    (void)e;
                                ...
                                const char *name, lv_event_cb_t cb)
{
    ...
    lv_obj_set_style_border_color(c,
        cb == launcher_card_stub_cb ? CD_PRIMARY_DIM : CD_PRIMARY,
        LV_PART_MAIN);
...
static void launcher_on_create(deck_bridge_ui_activity_t *act, void *intent_data)
{
    ...
    /* Real app card. */
    make_app_card(area, "[#]", "SETTINGS", launcher_card_settings_cb);
    /* Stubs for visual variety / future apps. */
    make_app_card(area, "[?]", "BOOKS",    launcher_card_stub_cb);
    make_app_card(area, "[?]", "NOTES",    launcher_card_stub_cb);
    make_app_card(area, "[?]", "FILES",    launcher_card_stub_cb);
    make_app_card(area, "[?]", "WIFI",     launcher_card_stub_cb);
}
...
deck_err_t deck_shell_dl2_boot(void)
{
    ...
    /* Settings registers its intent resolver early so the launcher
     * (and any other app) can navigate to it. */
